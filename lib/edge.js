...
fs.exists(path.resolve(__dirname, '../build/Release/edge_nativeclr.node'), (result2) =>{
  let builtEdge = path.resolve(__dirname, '../build/Release/' + (process.env.EDGE_USE_CORECLR || !result2 ? 'edge_coreclr.node' : 'edge_nativeclr.node'))
  let edge;
  ...
  fs.exists(builtEdge, (result1)=>{
    ...
    fs.exists(path.join(__dirname, 'bootstrap', 'bin', 'Release', 'netcoreapp1.1', 'bootstrap.dll'), (result) =>{
      if (process.env.EDGE_USE_CORECLR && !process.env.EDGE_BOOTSTRAP_DIR && result) {
...
exports.func = function(language, options) {
    ...
    if (typeof language !== 'string') {
        ...
        if (process.env.EDGE_USE_CORECLR) {
            ...
            fs.exists(compilerManifest, (result) => {
